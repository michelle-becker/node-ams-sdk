...
AzureMediaService.prototype.createEncodingJob = function ( options, cb ) {
  ...
  self.getAsset(options.assetId, function (err, res){
    
    if ( err || res.statusCode !== 200 ) return cb(err || JSON.parse(res.body))

    var asset = JSON.parse(res.body).d
    var task  = "<?xml version=\"1.0\" encoding=\"utf-8\"?><taskBody><inputAsset>JobInputAsset(0)</inputAsset><outputAsset"

    if ( options.outputName ) task += " assetName='" + options.outputName +"'"

    task += ">JobOutputAsset(0)</outputAsset></taskBody>"
