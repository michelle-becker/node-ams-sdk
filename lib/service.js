...
AzureMediaService.prototype._buildTaskXML = function ( options, inc ) {
  ...
  if ( options.encoding === 'Thumbnails' ){

    task = "<?xml version=\"1.0\" encoding=\"utf-8\"?><Thumbnail Size=\"100%,*\" Type=\"{%type%}\" Filename=\"{OriginalFilename}_{Size}_{ThumbnailTime}_{ThumbnailIndex}_{Date}_{Time}.{DefaultExtension}\"><Time Value=\"{%value%}\""
    ...
    task = "<?xml version=\"1.0\" encoding=\"utf-8\"?><taskBody><inputAsset>JobInputAsset(0)</inputAsset><outputAsset assetName=\"{%outputName%}\">JobOutputAsset({%inc%})</outputAsset></taskBody>"
...
AzureMediaService.prototype.createEncodingJob = function ( options, cb ) {

  if ( !(options instanceof Object) || typeof(options) === 'function' ) console.log('Must supply an options object')

  if ( typeof(cb) === 'undefined' ) console.log('Must supply a callback')

  var self = this

  this.getAsset(options.assetId, function (err, res){
    ...
    if (options.encoding === 'Thumbnails') {
      options.encoding = self._buildTaskXML(options)
    }
...
AzureMediaService.prototype.createMultiTaskJob = function ( options, cb ) {
  ...
  this.getAsset(options.assetId, function (err, res){
    ...
    options.tasks.forEach(function (task) {
      ...
      if (task.encoding === 'Thumbnails') {
        task.encoding = self._buildTaskXML(task)
      }

      data.Tasks.push({
        Configuration:    task.encoding,
        MediaProcessorId: self.MediaProcessorId,
